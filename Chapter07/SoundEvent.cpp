...
bool SoundEvent::IsValid() { return (mSystem && mSystem->GetEventInstance(mID) != nullptr); }
...
void SoundEvent::SetParameter(const std::string& name, float value) {
	auto event = mSystem ? mSystem->GetEventInstance(mID) : nullptr;
	if (event) event->setParameterValue(name.c_str(), value);
}

bool SoundEvent::GetPaused() const {
	bool retVal = false;
	auto event = mSystem ? mSystem->GetEventInstance(mID) : nullptr;
	if (event) event->getPaused(&retVal);
	return retVal;
}
...
float SoundEvent::GetVolume() const {
	float retVal = 0.0f;
	auto event = mSystem ? mSystem->GetEventInstance(mID) : nullptr;
	if (event) event->getVolume(&retVal);
	return retVal;
}
...
float SoundEvent::GetPitch() const {
	float retVal = 0.0f;
	auto event = mSystem ? mSystem->GetEventInstance(mID) : nullptr;
	if (event) event->getPitch(&retVal);
	return retVal;
}
...
float SoundEvent::GetParameter(const std::string& name) {
	float retVal = 0.0f;
	auto event = mSystem ? mSystem->GetEventInstance(mID) : nullptr;
	if (event) event->getParameterValue(name.c_str(), &retVal);
	return retVal;
}
...
bool SoundEvent::Is3D() const {
	...
	if (event) {
		...
		if (ed) ed->is3D(&retVal); //이 이벤트가 3D라면
	}
	return retVal;
}
