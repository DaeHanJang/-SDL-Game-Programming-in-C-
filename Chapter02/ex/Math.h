...
namespace Math {
	...
	inline float Cot(float angle) { return 1.f / Tan(angle); }
...
public:
	...
	static Matrix4 CreateFromQuaternion(const class Quaternion& q); //쿼터니언으로 회전 행렬 생성
	...
	static Matrix4 CreateLookAt(const Vector3& eye, const Vector3& target, const Vector3& up) {
		...
		float temp[4][4] = {
			{ xaxis.x, yaxis.x, zaxis.z, 0.f }, 
			{ xaxis.y, yaxis.y, zaxis.y, 0.f },
			{ xaxis.z, yaxis.z, zaxis.x, 0.f },
			{ xaxis.x, yaxis.y, zaxis.z, 1.f }
		};
		return Matrix4(temp);
	}
...
public:
	...
	//축과 각도로 쿼터니언 생성
	//축은 정규화되었다고 가정, 각도는 라디안
	explicit Quaternion(const Vector3& axis, float angle) {
		float scaler = Math::Sin(angle / 2.f);
		x = axis.x * scaler;
		y = axis.y * scaler;
		z = axis.z * scaler;
